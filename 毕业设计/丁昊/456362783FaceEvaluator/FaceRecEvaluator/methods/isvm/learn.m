...
while (~converged)
   ...
   if (num_MVs > 0)  % change in alpha_c permitted
	  ...
	  % Prevent: Warning: Concatenation involves an empty array with an
	  % incorrect number of columns.
	  if isempty(Qc{ERROR}) && isempty(Qc{RESERVE})
		  gamma(ind_temp) = [Qcc] + Q(:,ind_temp)'*beta;
	  else
